...
- **Dialog naming & placement convention**: dialog components follow the same `<code>_<Name>.vue` shape as screens but with a `D` code (`D0001`, `D0002`, ...) instead of `S`, one global sequence independent of the screen sequence — e.g. `D0001_StudySetPicker.vue` (not `StudySetPickerDialog.vue`). A dialog file is placed **next to whichever screen/component opens it** (e.g. [views/admin/D0001_StudySetPicker.vue](src/views/admin/D0001_StudySetPicker.vue) sits beside [views/admin/S0002_VideoVocab.vue](src/views/admin/S0002_VideoVocab.vue), which opens it via `dialog.showContent(...)` from [src/utilities/dialog.js](src/utilities/dialog.js)) — never in `src/components/`, which stays reserved for reusable, caller-agnostic UI. Full rules (including i18n): [.agents/skills/file_name/SKILL.md](.agents/skills/file_name/SKILL.md).
